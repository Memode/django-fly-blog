...
class Category(models.Model):
    ...
    desc = models.CharField(
        max_length=100, blank=True, verbose_name=u'描述', help_text=u'点击分类之后显示')
...
class Post(models.Model):
    ...
    tags = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=u'标签', help_text=u'用英文逗号分割')
    status = models.IntegerField(
        default=0, choices=STATUS.items(), verbose_name=u'状态')
    ...
    create_time = models.DateTimeField(
        u'创建时间', auto_now_add=True, editable=True)
    update_time = models.DateTimeField(u'更新时间', auto_now=True)
...
class Page(models.Model):
    ...
    alias = models.CharField(
        max_length=100, blank=True, null=True, verbose_name=u'英文标题', help_text=u'做伪静态url用')
    ...
    rank = models.IntegerField(
        default=1, verbose_name=u'排序', help_text=u'从右到左的顺序')
    status = models.IntegerField(
        default=0, choices=STATUS.items(), verbose_name=u'状态')
...
class Widget(models.Model):
    ...
    content = models.TextField(
        verbose_name=u'widget内容', help_text=u'"recently_post" and "hot_post" is special widgetr')
